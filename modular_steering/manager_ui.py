...
class Manager_ui(tk.Tk):
    def __init__(self, master, master_obj):
        ...
        self.b_add_module = tk.Button(self.frame_content, text = "add", command=self.edit_module)
        self.b_remove_module = tk.Button(self.frame_content, text = "remove", command=self.remove_callback)
        self.b_edit_module = tk.Button(self.frame_content, text = "edit", command=self.remove_callback)
    ...
    def geometry_manager(self):
        ...
        self.b_add_module.grid()
        self.b_remove_module.grid()
        self.b_edit_module.grid()
        self.b_test.grid()

        self.selected_modules_listbox.grid()
    ...
    def add_module(self, module):
        module.get_ui().grid(row = 0, column = 3, sticky = "nw", pady = 2)
    ...
    def add_callback(self, options):
        #popup = Popup()
        print(popup.get_value(options))
        self.selected_modules_listbox.insert(1, self.drop_down.get())
